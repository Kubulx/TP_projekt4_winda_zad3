# Symulator windy w C++ z użyciem GDI+
...
Program symuluje windę w budynku o 4 piętrach, poruszającą się pionowo i przewożącą pasażerów pomiędzy piętrami.

## Treść kodu i opis działania
Pliki nagłówkowe i konfiguracja
...
```
Stałe globalne związane z logiką symulacji
```cpp
...
```
Stałe globalne związane z wyświetlaniem symulacji
```cpp
...
```
Zmienne do płynnego wyświetlania symulacji
```cpp
HDC memDC = nullptr;
...
```
Funkcja odpowiada na którym piętrze jest winda
```cpp
...
```
Funkcja porządkuje pasażerów, by umożliwić ich poprawne wyświetlenie w symulacji
```cpp
void DecideNextMove() {
...
```
Funkcja przechodzi przez wszystkich pasażerów i tworzy listy zadań
```cpp
...
```
W przypadku braku zadań winda przechodzi stan IDLE, a funkcja kończy swoje działanie
```cpp
    bool is_full = (passengers_inside >= FULL_ELEVATOR_THRESHOLD);
```
Zmienna sprawdzająca, czy winda jest pełna, czy nie
```cpp
...
```
Winda szuka najbliższego wezwania
```cpp
 for (int i = 0; i < 2; ++i) {
        int next_stop = -1;
...
```
Winda szuka wezwań zgodnie ze swoim kierunkiem jazdy
```cpp
...
```
Jeśli winda nie znajdzie żadnych wezwań przechodzi w stan IDLE
```cpp
void MoveElevator() {
...
```
Część odpowiedzialna za animacje wsiadania pasażerów do windy
```cpp
    switch (currentState) {
...
```
Obsługa windy w stanie IDLE. Winda reaguje na nowe wezwania. W przypadku ich braku wraca na parter
```cpp
...
```
Winda przemieszcza się do piętra docelowego
```cpp
...
```
Winda wymienia pasażerów na piętrze. Pasażer może wejść, jeśli kierunek jego jazdy jest zgodny z kierunkiem windy. 
Na końcu winda na nowo ustala swoje priorytety.
## Funkcja rysujaca pasazera
Funkcja rysuje pasazera - czlowieka w formie "patyczaka"
...
```
## Funkcja rysujaca dzialanie programu
Funkcja DrawScene rysuje całą scenę symulacji windy:
- rysuje ścianę szybu z podziałem na piętra oraz podpisami „Piętro n”,
- rysuje podłogę na każdym piętrze i samą windę (kabina z drzwiami),
- wyświetla pasażerów – na piętrach, wsiadających lub już wewnątrz windy – wraz z ich docelowym piętrem,
- pokazuje aktualną masę w windzie i krótką instrukcję dla użytkownika na dole ekranu.
...
## Funkcja obsługująca zdarzenia w oknie programu
- WM_ERASEBKGND – zapobiega miganiu obrazu.
- WM_SIZE – przygotowuje nowe miejsce do rysowania, gdy okno zmienia rozmiar.
- WM_TIMER – przesuwa windę i odświeża ekran.
- WM_COMMAND – dodaje pasażera po kliknięciu przycisku.
- WM_PAINT – rysuje cały widok windy i pasażerów na ekranie.
- WM_DESTROY – zwalnia pamięć i zamyka program.
...
- Uruchamia GDI+, potrzebne do rysowania grafiki.
- Rejestruje okno aplikacji i tworzy główne okno o odpowiednim rozmiarze.
- Dodaje przyciski do wywoływania pasażerów z danego piętra na inne piętra.
- Ustawia timer, który co 33 ms odświeża scenę i porusza windę.
- Obsługuje główną pętlę komunikatów, czyli reaguje na zdarzenia w programie (kliknięcia, rysowanie itd.).
